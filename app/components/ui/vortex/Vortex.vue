...
const offscreen = shallowRef<HTMLCanvasElement | null>(null);
const offCtx = shallowRef<CanvasRenderingContext2D | null>(null);
...
function updateParticle(i: number) {
  if (!particleProps.value || !canvasRef.value || !offCtx.value) return;

  const canvas = canvasRef.value;
  const props = particleProps.value;
  const context = offCtx.value;
...
function draw() {
  if (!canvasRef.value || !ctx.value || !offscreen.value || !offCtx.value || !particleProps.value) return;

  const canvas = canvasRef.value;
  const context = ctx.value;
  const off = offscreen.value;
  const oCtx = offCtx.value;

  tick.value++;

  // Clear offscreen (transparent)
  oCtx.clearRect(0, 0, off.width, off.height);

  // Draw particles on offscreen
  for (let i = 0; i < particleProps.value.length; i += PARTICLE_PROP_COUNT) {
    updateParticle(i);
  }

  // Apply glow on offscreen only (no background affected)
  oCtx.save();
  oCtx.filter = "blur(8px) brightness(200%)";
  oCtx.globalCompositeOperation = "lighter";
  oCtx.drawImage(off, 0, 0);
  oCtx.restore();

  oCtx.save();
  oCtx.filter = "blur(4px) brightness(200%)";
  oCtx.globalCompositeOperation = "lighter";
  oCtx.drawImage(off, 0, 0);
  oCtx.restore();

  // Draw background then composite particles on top
  context.fillStyle = resolveColor(props.backgroundColor);
...
function resizeOffscreen(width: number, height: number) {
  if (!offscreen.value) return;
  offscreen.value.width = width;
  offscreen.value.height = height;
}
...
const handleResize = useDebounceFn(() => {
  ...
  resizeOffscreen(innerWidth, innerHeight);
...
onMounted(() => {
  ...
  offscreen.value = off;
  offCtx.value = off.getContext("2d");

  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;
  resizeOffscreen(canvas.width, canvas.height);
  center.value = [0.5 * canvas.width, 0.5 * canvas.height];
